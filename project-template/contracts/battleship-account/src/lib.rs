...
impl BattleshipAccount {
    ...
    pub fn place_ship(&mut self, row: Felt, col: Felt, ship_id: Felt) {
        let config: Word = self.game_config.read();
        assert!(config[2].as_u64() == PHASE_CREATED, "wrong phase");
        ...
        let num_placed = config[1].as_u64() + 1;
        self.game_config.write(Word::from([
            config[0],
            Felt::from_u64_unchecked(num_placed),
            config[2],
            config[3],
        ]));
    }
    ...
    pub fn finalize_setup(
        ...
        assert!(
            config[1].as_u64() == TOTAL_SHIP_CELLS,
        ...
        assert!(s1.as_u64() == SHIP_1_SIZE, "ship 1 wrong size");

        let s2: Felt = self
            .my_board
            .get(&Word::from([felt!(1), felt!(0), felt!(0), felt!(2)]));
        assert!(s2.as_u64() == SHIP_2_SIZE, "ship 2 wrong size");

        let s3: Felt = self
            .my_board
            .get(&Word::from([felt!(1), felt!(0), felt!(0), felt!(3)]));
        assert!(s3.as_u64() == SHIP_3_SIZE, "ship 3 wrong size");

        let s4: Felt = self
            .my_board
            .get(&Word::from([felt!(1), felt!(0), felt!(0), felt!(4)]));
        assert!(s4.as_u64() == SHIP_4_SIZE, "ship 4 wrong size");

        let s5: Felt = self
            .my_board
            .get(&Word::from([felt!(1), felt!(0), felt!(0), felt!(5)]));
        assert!(s5.as_u64() == SHIP_5_SIZE, "ship 5 wrong size");

        // Store game_id
        self.game_id.write(game_id);

        // Store opponent info
        self.opponent.write(Word::from([
            opponent_prefix,
            opponent_suffix,
            felt!(0),
            felt!(0),
        ...
        // Store commitment
        self.board_commitment.write(commitment);

        // Set phase to CHALLENGED
        self.game_config.write(Word::from([
            Felt::from_u64_unchecked(GRID_SIZE),
            Felt::from_u64_unchecked(TOTAL_SHIP_CELLS),
            Felt::from_u64_unchecked(PHASE_CHALLENGED),
            felt!(0),
        ]));
    }
    ...
    pub fn accept_challenge(
        ...
        self.opponent_commitment.write(opponent_commitment);
        ...
        self.game_config.write(Word::from([
            Felt::from_u64_unchecked(GRID_SIZE),
            Felt::from_u64_unchecked(TOTAL_SHIP_CELLS),
            Felt::from_u64_unchecked(PHASE_ACTIVE),
            felt!(1),
        ]));
    }
    ...
    pub fn receive_acceptance(
        ...
        // Store opponent commitment
        self.opponent_commitment.write(acceptor_commitment);
    ...
    pub fn process_shot(&mut self, row: Felt, col: Felt, turn: Felt) -> Felt {
        let config: Word = self.game_config.read();
        assert!(config[2].as_u64() == PHASE_ACTIVE, "wrong phase for shot");

        // Validate turn
        assert!(turn.as_u64() == config[3].as_u64(), "wrong turn number");

        // Check bounds
        assert!(row.as_u64() < GRID_SIZE, "row out of bounds");
        assert!(col.as_u64() < GRID_SIZE, "col out of bounds");

        // Read cell
        let key = Word::from([felt!(0), felt!(0), row, col]);
        let cell: Felt = self.my_board.get(&key);
        let cell_val = cell.as_u64();

        // Check not already shot
        assert!(cell_val != CELL_HIT, "cell already hit");
        assert!(cell_val != CELL_MISS, "cell already shot");

        // Read opponent info
        let opp: Word = self.opponent.read();
        let ships_hit_count = opp[2].as_u64();
        let total_shots = opp[3].as_u64();
        ...
        // Update cell
        self.my_board.set(key, Felt::from_u64_unchecked(new_cell));

        // Update opponent info (counters)
        self.opponent.write(Word::from([
            opp[0],
            opp[1],
            Felt::from_u64_unchecked(new_hit_count),
            Felt::from_u64_unchecked(total_shots + 1),
        ]));
        ...
        self.game_config.write(Word::from([
            Felt::from_u64_unchecked(GRID_SIZE),
            Felt::from_u64_unchecked(TOTAL_SHIP_CELLS),
            Felt::from_u64_unchecked(new_phase),
            Felt::from_u64_unchecked(config[3].as_u64() + 2),
        ]));

        // Return encoded: result * 2 + game_over
        Felt::from_u64_unchecked(result * 2 + game_over)
    }
    ...
    pub fn enter_reveal(&mut self) {
        ...
        self.game_config.write(Word::from([
            config[0],
            config[1],
            Felt::from_u64_unchecked(PHASE_REVEAL),
            config[3],
        ]));
    }
    ...
    pub fn mark_my_reveal(&mut self) {
        let config: Word = self.game_config.read();
        assert!(
            config[2].as_u64() == PHASE_REVEAL,
            "wrong phase for mark_my_reveal"
        );

        let status: Word = self.reveal_status.read();
        self.reveal_status.write(Word::from([
            felt!(1),
            status[1],
            felt!(0),
            felt!(0),
        ...
        if status[1].as_u64() == 1 {
            self.game_config.write(Word::from([
                config[0],
                config[1],
                Felt::from_u64_unchecked(PHASE_COMPLETE),
                config[3],
            ]));
        }
    }
    ...
    pub fn verify_opponent_reveal(&mut self, commitment: Word) {
        ...
        let stored: Word = self.opponent_commitment.read();
